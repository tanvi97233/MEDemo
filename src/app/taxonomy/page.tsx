...
export default async function TaxonomyPage({ searchParams }: { searchParams: Promise<{ q?: string; programme?: string }> }) {
  ...
  const [nodes, programmes]: [any[], any[]] = await Promise.all([
  ...
  const childrenOf = (id: string) => nodes.filter((node: any) => node.parentId === id);
  const linkedIds = new Set<string>();
  for (const node of nodes.filter((item: any) => item.programmes.length)) {
    linkedIds.add(node.id);
    if (node.parentId) linkedIds.add(node.parentId);
    const parent = nodes.find((item: any) => item.id === node.parentId);
    if (parent?.parentId) linkedIds.add(parent.parentId);
  }
  const sectors = nodes.filter((node: any) => node.level === "SECTOR" && linkedIds.has(node.id));
  const focusNodes = nodes.filter((node: any) => node.level === "SUB_SUB_SECTOR");
  ...
  return <>
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_380px]">
      <div className="space-y-4">{sectors.map((sector: any) => {
        const subsectors = childrenOf(sector.id).filter((child: any) => linkedIds.has(child.id));
        const visible = !query || sector.name.toLowerCase().includes(query) || subsectors.some((child: any) => child.name.toLowerCase().includes(query) || childrenOf(child.id).some((focus: any) => focus.name.toLowerCase().includes(query)));
        if (!visible) return null;
        return <details key={sector.id} open className="rounded-2xl border border-slate-200 bg-white"><summary className="cursor-pointer p-5 font-bold text-slate-950">{sector.name} <Badge tone="blue">Sector</Badge></summary><div className="space-y-3 border-t border-slate-100 p-5">{subsectors.map((subsector: any) => <details key={subsector.id} open className="rounded-xl border border-slate-200"><summary className="cursor-pointer p-4 font-semibold">{subsector.name} <Badge>Sub-sector</Badge></summary><div className="grid gap-3 border-t border-slate-100 p-4 sm:grid-cols-2">{childrenOf(subsector.id).filter((focus: any) => linkedIds.has(focus.id)).map((focus: any) => <Card key={focus.id} className="p-4"><div className="flex justify-between gap-2"><h3 className="font-semibold">{focus.name}</h3><Badge>Sub-sub-sector</Badge></div><p className="mt-3 text-xs font-bold uppercase text-slate-400">Linked programmes</p>{focus.programmes.map((link: any) => <div className="mt-2" key={link.programmeId}><p className="text-sm font-medium">{link.programme.name}</p><p className="text-xs text-slate-500">{link.programme.indicators.length} linked indicators</p></div>)}</Card>)}</div></details>)}</div></details>;
      })}</div>
